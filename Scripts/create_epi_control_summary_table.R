...
# FUNCTIONS ---------------------------------------------------------------
 ...
 # Subset to countries for 1 table per country
  ...
  create_epi_tbl <- function(sel_cntry = "Zambia") { 
    bind_rows(
      df_95s %>% filter_cntry(sel_cntry) %>% select(country, indicator, col1 = known_status, col2 = on_art, col3 = vls),   
      df_epi_cntrl %>% filter_cntry(sel_cntry) %>% select(country, indicator, col3 = epi_control),
      df_epi_stats %>% filter_cntry(sel_cntry) %>% select(country, indicator = name, col3 = value_formatted),
    ) %>% 
      gt() %>% 
      text_transform(
  ...
  prepare_95s_summary <- function(data) {
    data %>%
      filter(indicator %in% c("Percent Known Status of PLHIV", "Percent on ART with Known Status", "Percent VLS on ART")) %>%
      select(country, iso, indicator, estimate) %>%
      arrange(country, indicator) %>%
      group_by(country) %>%
      mutate(
        indicator = recode(indicator, "Percent Known Status of PLHIV" = "Known Status", 
...
# MUNGE ------------------------------------------------------------------
  ...
  create_epi_tbl()
  cntry_list <- df %>% distinct(country) %>% slice(1:10) %>% pull()
