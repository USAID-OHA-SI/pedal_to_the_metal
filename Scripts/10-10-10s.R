...
# LICENSE: MIT
# DATE:   2024-11-14
# NOTES: Updated title and caption. adapted from https://github.com/USAID-OHA-SI/hardapoart/blob/main/Scripts/16_10s_barriers.R
...
dotplot_viz_10s <- function(df, cntry, export = TRUE) {
  ...
  v <- df %>% 
    ...
    ggplot2::labs(x = "Status", y = NULL,
                  title = "GAPS IN THE UNAIDS 10-10-10 GOALS: POLICY & LAWS",
                  ...
                  caption = "Note: These indicators measure sub-target towards the first 10. Second and third 10S are best measured using PHIA, DHS, or IBBS data.") +
    #subtitle = glue::glue("{unique(df$country)}'s progress towards adopting structural laws/policies towards UNAIDS' 10-10-10 goals"),
    #caption = glue::glue("{metadata_pol_lab$caption} |  Ref id: {ref_id}")) +
    glitr::si_style_nolines() +
